...
public class LrWithExplanation implements Comparable<LrWithExplanation> {
    ...
    String getEscapedExplanation(Ontology ontology) {
        ...
        switch (this.matchType) {
            case EXACT_MATCH:
                return String.format("E: %s LR: %.3f", qtermlabel, LR);
            case QUERY_TERM_SUBCLASS_OF_DISEASE_TERM:
                return String.format("Q&lt;D: %s&lt;%s LR: %.3f", qtermlabel, mtermlabel, LR);
            case DISEASE_TERM_SUBCLASS_OF_QUERY:
                return String.format("D&lt;Q: %s&lt;%s LR: %.3f", mtermlabel, qtermlabel, LR);
            case NON_ROOT_COMMON_ANCESTOR:
                return String.format("Q~D:%s~%s LR: %.3f", qtermlabel, mtermlabel, LR);
            case NO_MATCH_BELOW_ROOT:
                return String.format("NM:%s LR: %.3f", qtermlabel, LR);
            case QUERY_TERM_PRESENT_BUT_EXCLUDED_IN_DISEASE:
                return String.format("X:%s LR: %.3f", qtermlabel, LR);
            case UNUSUAL_BACKGROUND_FREQUENCY:
                return String.format("U:%s LR: %.3f", qtermlabel, LR);
            case EXCLUDED_QUERY_TERM_EXCLUDED_IN_DISEASE:
                return String.format("XX:%s LR: %.3f", qtermlabel, LR);
            case EXCLUDED_QUERY_TERM_NOT_PRESENT_IN_DISEASE:
                return String.format("XA:%s LR: %.3f", qtermlabel, LR);
            case EXCLUDED_QUERY_TERM_PRESENT_IN_DISEASE:
                return String.format("XP:%s LR: %.3f", qtermlabel, LR);
            default:
                return String.format("NM:%s LR: %.3f", qtermlabel, LR); // should never happen but needed for compiler
        }
    }
