...
public class GenicIntoleranceCalculator {
    ...
    public GenicIntoleranceCalculator(VariantAnnotator variantAnnotator, MVStore alleleStore, VariantDataService variantDataService) {
        this.variantAnnotator = variantAnnotator;
        this.alleleMap = MvStoreUtil.openAlleleMVMap(alleleStore);
        this.variantDataService = variantDataService;
    }
    /** Count of variants with non-existent pathogenicity prediction data, for debugging. */
    private int pathDataEmpty = 0;
    /** Key: A gene symbol ; value: the corresponding  object for the background ("population") variants. */
...
//            initBins();
            ...
            outputBinData();
        } else if (do1000genomes) {
            binThousandGenomesVcfData();
        }
    }
    ...
    private void binThousandGenomesVcfData() {
        logger.info("Binning 1K Genomes VCF data");
...
//        }
     ...
     *
     * aram g2bmap The bins data for background (gnomAD, TOPMed, etc from Exomiser database)
     */
